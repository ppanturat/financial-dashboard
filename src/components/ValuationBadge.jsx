 ...
 * ValuationBadge.jsx
 * ─────────────────────────────────────────────────────────────────────────────
 * Micro-valuation indicator for individual stocks.
 * Compares current Forward P/E to the historical median P/E and flags:
 *   • "Deep Discount"   — current multiple ≥ 20% below historical median
 *   • "Heavy Premium"   — current multiple ≥ 25% above historical median
 *   • "Fair Value"      — within that band
 *
 * Data comes from the /api/valuation/:ticker endpoint.
 *
 * Usage:
 *   <ValuationBadge ticker="AAPL" />
 */
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        <div>
          <div style={{ fontSize: 10, color: '#6b6a65', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
            Fwd P/E
          </div>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 700, color: cfg.color }}>
            {currentPE ?? '—'}×
          </div>
        </div>
        ...
        <div>
          ...
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 13, fontWeight: 700, color: '#111' }}>
            {medianPE ?? '—'}×
          </div>
        </div>
        {discountPct && (
      ...
      {/* Detail text */}
      <p style={{ margin: 0, fontSize: 12, color: '#6b6a65', lineHeight: 1.5 }}>
        {cfg.detail}
      </p>
    </div>
  )
